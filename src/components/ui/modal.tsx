...
/**
 * Universal Motion Modal component powered by Radix UI & Motion.
...
export function Modal({
  ...
  return (
    <DialogPrimitive.Root open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <AnimatePresence>
        {isOpen && (
          <DialogPrimitive.Portal forceMount data-slot="modal-portal">
            {/* Backdrop Overlay */}
              ...
              >
                {(title || description) && (
                  <div className="flex flex-col gap-1.5 text-left pr-8">
                    {title && (
                      <DialogPrimitive.Title className="font-semibold text-lg leading-tight tracking-tight text-foreground">
                        {title}
                ...
                {showCloseButton && (
                  <DialogPrimitive.Close
                    ...
                    className="absolute top-4 right-4 flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground opacity-70 transition-all hover:bg-accent hover:text-foreground hover:opacity-100 focus:outline-hidden focus:ring-2 focus:ring-primary"
